...
def get_files():
    ...
    label.config(text="Available Dates in this report are: ")

    for n in uniq:
        if "20" in n:
            date_list.insert(tk.END,n)
...
def process_csv_files():
    ...
    #input_text = entry.get()
    #thedate = input_text
    thedate = date_list.get(tk.ACTIVE)
    if len(thedate) < 10:
...
words_text = tk.Text(window, height=10, width=50,state="disabled")
date_list=tk.Listbox(window,height=5,width=20)
